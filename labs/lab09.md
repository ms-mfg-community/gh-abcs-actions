# 9 - AI Inference with GitHub Actions
...
## 9.1 Add AI Inference to a Workflow
...
4. Commit the changes into a new `feature/lab09` branch and push to the remote repository
...
## 9.2 Prompt Files and Templating
...
**Second**, replace the `Run AI Inference` step that we added in 9.1 with the following (keep the `Display Response` step):
...
5. Go to **Actions** and select the **09-1. AI Inference** workflow
...
## 9.3 Structured Output (JSON Schema)
...
## 9.4 Repository Context with MCP (Advanced - Optional)

> **This section is optional.** It requires creating a Personal Access Token (PAT) and storing it as a repository secret. If you're not comfortable with PAT management or want to skip advanced content, proceed directly to section 9.5 (Final).
...
3. Create an issue with:
   - **Title:** `TEST-MCP-VERIFY: Lab 9 MCP Test Issue`
...
2. Go to **Actions** → select **09-1. AI Inference** workflow
3. Click **Run workflow**
4. Wait for completion and check the **mcp-demo** job:
   - Verify the job ran (not skipped)
   - Expand **Display MCP Response**
   - Look for your test issue: `TEST-MCP-VERIFY: Lab 9 MCP Test Issue`
...
1. **Delete the PAT:** GitHub Settings → Developer settings → Personal access tokens → Delete `lab9-mcp-token`
...
## 9.6 Auto-Analyze Failures (Hands-On)
...
Create a new workflow file at `.github/workflows/lab09-intentional-failure.yml` with the following contents:

```yaml
name: 09-3. Intentional Failure (for Auto-Analyze)
  ...
  fail-on-purpose:
    ...
    steps:
      ...
      - name: Fail intentionally
        run: |
          echo "::error::Intentional failure for Lab 9.6"
          exit 1
```
...
1. Go to **Actions**
2. Select **09-3. Intentional Failure (for Auto-Analyze)**
3. Click **Run workflow**
4. Wait for the run to fail (this is expected)

### Step 3: Watch the Auto-Analyze Workflow Trigger

After the failure completes, GitHub will automatically start a new workflow run for **09-2. Auto Analyze Build Failures**.

1. Go to **Actions** → select **09-2. Auto Analyze Build Failures**
...
1. Update `.github/workflows/lab09-intentional-failure.yml` to stop failing (remove the failing step or change `exit 1` to `exit 0`).
2. Commit and push.
3. Re-run **09-3. Intentional Failure (for Auto-Analyze)** and confirm it succeeds.
...
- Delete `.github/workflows/lab09-intentional-failure.yml` after the lab
